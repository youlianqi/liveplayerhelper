...
- (void)scrollToPageWithIndex:(NSInteger)index {
    //NSLog(@"GKVideoScrollView scrollToPageWithIndex index:%@", @(index));
...
- (void)scrollToLastPage {
    //NSLog(@"GKVideoScrollView scrollToNextPage");
...
- (void)removeCurrentPageAnimated:(BOOL)animated {
    ...
    //NSLog(@"GKVideoScrollView removeCurrentPageAnimated");
...
- (void)removeCurrentPage {
    ...
    //NSLog(@"GKVideoScrollView removeCurrentPage");
    ...
    if (self.totalCount >= 3) {
        if (index == 0) {
            ...
            if (self.currentCell && self.currentCell == self.btmCell) {
                ...
                [self updateContentOffset];
                [self updateUpScrollCellWithIndex:index scrollView:self];
            }
        }
    }
    [self createCellWithType:type index:index];
...
- (void)updateDisplayCell:(BOOL)isFirstLoad {
    ...
    //NSLog(@"GKVideoScrollView updateDisplayCell");
...
- (void)updateDisplayCellWithIndex:(NSInteger)index {
    //NSLog(@"GKVideoScrollView updateDisplayCellWithIndex index:%@",@(index));
...
- (void)updateContentOffset {
    //NSLog(@"GKVideoScrollView updateContentOffset");
...
/// 上滑cell
- (void)updateUpScrollCellWithIndex:(NSInteger)index scrollView:(UIScrollView *)scrollView  {
    ////NSLog(@"GKVideoScrollView updateUpScrollCellWithIndex index:%@", @(index));
    ...
    [self didUpdateScrollCell:self.ctrCell forIndex:index];
    [self scrollViewDidEndScrollingAnimation:scrollView];
}

/// 下滑cell
- (void)updateDownScrollCellWithIndex:(NSInteger)index scrollView:(UIScrollView *)scrollView {
    ////NSLog(@"GKVideoScrollView updateDownScrollCellWithIndex index:%@", @(index));
    if (index < 1 || index > self.totalCount - 2) return;
    // 下视图放入复用池
    [self saveReusableCell:self.btmCell];
    // 中视图切换为下视图
...
- (void)delayUpdateCellWithIndex:(NSInteger)index {
    //NSLog(@"GKVideoScrollView delayUpdateCellWithIndex index:%@", @(index));
    ...
    //dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (index != NSNotFound) {
            ...
            [self updateUpScrollCellWithIndex:self.index scrollView:self];
...
- (void)willDisplayCell:(GKVideoViewCell *)cell forIndex:(NSInteger)index {
    //NSLog(@"GKVideoScrollView willDisplayCell index:%@",@(index));
...
- (void)didEndScrollingCell:(GKVideoViewCell *)cell forIndex:(NSInteger)index {
    //NSLog(@"GKVideoScrollView didEndScrollingCell index:%@ begin", @(index));
    if (index < 0 || index >= self.totalCount) return;
    if (!cell) return;
    ...
    //NSLog(@"GKVideoScrollView didEndScrollingCell index:%@ end", @(index));
}

- (void)didEndScrollingCell:(GKVideoViewCell *)cell {
    //NSLog(@"GKVideoScrollView didEndScrollingCell begin");
    ...
    //NSLog(@"GKVideoScrollView didEndScrollingCell end 1");

    // 更新滑动结束时显示的cell
    [self didEndScrollingCell:cell forIndex:self.currentIndex];
    
    //NSLog(@"GKVideoScrollView didEndScrollingCell end 2");

}

- (void)didUpdateScrollCell:(GKVideoViewCell *)cell forIndex:(NSInteger)index {
    if (index < 0 || index >= self.totalCount) return;
    if (!cell) return;
    if ([self.userDelegate respondsToSelector:@selector(scrollView:didEndScrollingCell:forRowAtIndexPath:)]) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:index inSection:0];
        [self.userDelegate scrollView:self didUpdateScrollCell:cell forRowAtIndexPath:indexPath];
    }
}
    ...
    if (offsetY >= 2 * viewH) { // 上滑
        ...
        if (self.index == 0) {
            if (self.lastCount > 0) {
                ...
                [self updateUpScrollCellWithIndex:self.index scrollView:scrollView];
            }
        }else {
        ...
        if (self.index == 1) {
            self.index -= 1;
            self.changeIndex = self.index;
            [self updateDownScrollCellWithIndex:self.index scrollView:scrollView];
